...
Each package config file is a simple attribute set passed to the package's wrapper. Example `config/tealdeer.nix`:
```nix
{
...
## How It Works
1. For each package, customix extracts its name `pname` or `name`)
2. Looks for a config file at `configDir/<name>.nix`
3. If found, loads and merges the config with package arguments
4. Passes the merged config to `nix-wrapper-modules` for wrapping
